...
```
Congrats! Your dataframes are now formatted in a way that makes sense, and the data are ready for you to make plots and/or run statistics. If you would like to combine all of your spectra data frames into a single dataframe, you can use the following code below
```{r}
...
```
##3.Background subtract noise from raw spectra data
The first step in analyzing MALDI-TOF raw spectra data is to distinguish the
signal from the noise in the mass spec. readings.
```{r}
test<-import("2309_3.1.txt")
